...
def constructQuery(qc):
    ...
    #qc.addSelect("measurement","date_part('hour',\"localtime\")")
    qc.addSelect("warmup_experiment","cast((highest/100) as int)")
    #qc.addWhereRaw("cast(signalstrength as int)>0")
    qc.addWhereNotEqualsString("device","networkname","")    
    qc.applyMobileClauses()
    qc.applyWarmupClauses()
...
#i+=1
orange.add(Column("highest",i,"continuous",True))
i+=1
