...
def package_profile_over_time_of_day(package):
	return "select date_part('hour', \"localtime\"),cast(floor(date_part('minute', \"localtime\")/10) as int),sum(total_diff),count(*) from measurement,application_use where measurement.measurementid = application_use.measurementid and package='"+ str(package) + "' and total_diff>0 group by date_part('hour', \"localtime\"),cast(floor(date_part('minute', \"localtime\")/10) as int) order by date_part('hour', \"localtime\"),cast(floor(date_part('minute', \"localtime\")/10) as int)"
...
def ping_over_time_finegrain():
	return "select dstip,networktype,date_part('hour', \"localtime\"),cast(floor(date_part('minute', \"localtime\")/10) as int),avg(\"avg\"),avg(stdev) 	from measurement,ping,network where avg<600 and dstip!='localhost' and networktype!='UNKNOWN' and networktype!='' and networktype!='GPRS' and networktype!='HSPA' and connectiontype!='' and connectiontype!='Wifi' and measurement.measurementid = network.measurementid and measurement.measurementid = ping.measurementid and avg>0	group by networktype,dstip,date_part('hour', \"localtime\"),cast(floor(date_part('minute', \"localtime\")/10) as int)	order by date_part('hour', \"localtime\"),cast(floor(date_part('minute', \"localtime\")/10) as int);"
